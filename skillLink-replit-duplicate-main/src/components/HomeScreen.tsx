  ...
  return (
    <div className="bg-gradient-to-r from-orange-400 to-red-500 rounded-3xl p-6 flex items-center gap-5 shadow-lg mb-4 animate-pulse">
      ...
      <div className="flex-1">
        <p className="text-white font-bold text-lg">Streak Reminder! ðŸ”¥</p>
        <p className="text-orange-100 text-lg">You haven't completed a quest today â€” don't break your 7-day streak!</p>
      </div>
      <button
        onClick={onDismiss}
...
export function HomeScreen({ onNavigate, userProfile, userPoints, skillLevels, weeklyEvent, language = 'en', assignedQuests = [] }: HomeScreenProps) {
  ...
  const quickActions = [
    ...
    { id: 'courses', label: t.courses, icon: BookOpen, color: 'from-purple-400 to-purple-500', count: 'ðŸ“š' },
    { id: 'skill-tree', label: 'Skill Tree', icon: TrendingUp, color: 'from-green-400 to-emerald-500', count: 'ðŸŒŸ' },
    { id: 'statistics', label: 'Statistics', icon: BarChart2, color: 'from-indigo-400 to-indigo-600', count: 'ðŸ“ˆ' },
  ];

  const recentBadges = [
    { name: 'Early Bird', emoji: 'ðŸŒ…' },
    { name: 'Creative', emoji: 'ðŸŽ¨' },
    { name: 'Book Worm', emoji: 'ðŸ“š' },
  ];
    ...
    <div className="h-full bg-background pb-20 overflow-y-auto">
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <h2 className="text-primary-foreground font-bold">
              {userProfile?.name || 'Friend'}! {userProfile?.avatar || 'ðŸ‘‹'}
            </h2>
          </div>
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          >
            <div className="flex items-center justify-center gap-1 mb-1">
              <SkillCoin size={20} />
              <span className="text-white text-lg">{t.points}</span>
            </div>
          ...
          >
            <div className="flex items-center justify-center gap-1 mb-1">
              ...
              <span className="text-white text-lg">{t.streak}</span>
            </div>
            <p className="text-white text-2xl font-bold">7 {language === 'es' ? 'dÃ­as' : language === 'fr' ? 'jours' : language === 'de' ? 'Tage' : language === 'ja' ? 'æ—¥' : language === 'zh' ? 'å¤©' : 'days'}</p>
          </button>
          <button
          ...
          >
            <div className="flex items-center justify-center gap-1 mb-1">
              ...
              <span className="text-white text-lg">{t.badges}</span>
            </div>
      ...
      <div className="px-6 mt-5">
        <button
          ...
          className="w-full bg-gradient-to-r from-purple-600 to-pink-500 rounded-3xl p-6 flex items-center gap-6 shadow-lg active:scale-95 transition-transform"
        >
          <div className="bg-white/20 rounded-2xl p-2.5">
            <Crown className="text-yellow-300" size={24} />
          </div>
          <div className="flex-1 text-left">
            <p className="text-white font-black text-lg">Unlock Premium âœ¨</p>
            <p className="text-purple-100 text-lg">Family Plan from â‚¬6.99/mo â€” Cancel anytime</p>
          </div>
          <div className="bg-white text-purple-600 font-black text-lg px-3 py-1.5 rounded-full">
            See Plans
          </div>
        </button>
      </div>
        ...
        <div className="bg-card rounded-3xl p-5 shadow-md border-2 border-border">
          ...
          <p className="text-muted-foreground mt-2 text-lg">
            {completedQuests === totalQuests && totalQuests > 0
              ? 'ðŸŽ‰ All quests complete! Amazing job!'
              : t.keepGoing + ' ðŸŽ‰'}
          </p>
        </div>
      </div>
          ...
          {quickActions.map((action) => (
            ...
            >
              ...
              <div className="mt-2 bg-white/20 rounded-full px-2 py-0.5 inline-flex items-center">
                <span className="text-white text-lg">{action.count}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-5">
          {recentBadges.map((badge) => (
            ...
            >
              ...
              <p className="text-lg text-muted-foreground">{badge.name}</p>
            </button>
          ))}
        </div>
      </div>
