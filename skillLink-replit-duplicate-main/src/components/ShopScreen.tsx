  ...
  if (item.gradientBorder) {
    return (
      <div className="p-1 rounded-full bg-gradient-to-r from-red-400 via-yellow-400 via-green-400 via-blue-400 to-purple-400 flex-shrink-0">
        <div className={`${cls} rounded-full bg-gray-100 flex items-center justify-center`}>ðŸ˜Š</div>
      </div>
    );
  }
  return (
  ...
  return (
    <div className="h-full bg-gradient-to-b from-purple-50 to-pink-50 pb-24 overflow-y-auto">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-pink-500 px-6 pt-12 pb-8 rounded-b-[3rem] shadow-xl">
        ...
        <p className="text-purple-100 text-lg mb-4">Spend your SkillCoins on borders and Robux! ðŸŽ</p>
        ...
        <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-5">
              <SkillCoin size={44} />
              <div>
                <p className="text-white/70 text-lg font-medium">SkillCoin Balance</p>
                <p className="text-white text-3xl font-black">{currentPoints.toLocaleString()}</p>
                <p className="text-yellow-300 text-lg">SC coins</p>
              </div>
            </div>
      ...
      <div className="px-6 mt-5">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-2 w-full bg-white rounded-2xl p-1 shadow-md border border-gray-100">
            <TabsTrigger value="borders" className="rounded-xl text-lg font-bold data-[state=active]:bg-purple-500 data-[state=active]:text-white">
              ðŸŽ¨ Borders
            </TabsTrigger>
            <TabsTrigger value="robux" className="rounded-xl text-lg font-bold data-[state=active]:bg-gray-900 data-[state=active]:text-white">
              <RobuxBadge size={18} />
          ...
          <TabsContent value="borders" className="mt-5 space-y-5">
            ...
            <div>
              ...
              <div className="space-y-3">
                {featuredBorders.map(item => {
                  ...
                  return (
                    <div key={item.id} className={`bg-white border-2 rounded-3xl p-6 shadow-md transition-all ${owned ? 'border-green-300 bg-green-50' : canAfford ? 'border-purple-200' : 'border-gray-100 opacity-80'}`}>
                      <div className="flex items-center gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-4 mb-0.5 flex-wrap">
                            ...
                            <Badge className="bg-yellow-400 text-yellow-900 text-[10px] px-2">â­ Featured</Badge>
                          </div>
                          <p className="text-gray-400 text-lg mb-2">{item.description}</p>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                              <SkillCoin size={18} />
                              <span className="text-gray-700 text-lg font-bold">{item.price} SC</span>
                            </div>
                            {owned ? (
                              <Badge className={`text-lg ${justBought ? 'bg-green-600 animate-pulse' : 'bg-green-500'}`}>
                                {justBought ? 'âœ“ Purchased!' : 'Owned âœ“'}
                              </Badge>
                            ) : canAfford ? (
                              <Button onClick={() => handleBuyBorder(item)} className="bg-purple-500 hover:bg-purple-600 text-white rounded-full px-5 h-8 text-lg font-bold active:scale-95">
                                Buy
                              </Button>
                            ) : (
                              <span className="text-lg text-gray-400 flex items-center gap-1"><Lock size={11} /> Need more SC</span>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-5">
                {regularBorders.map(item => {
                  ...
                  const justBought = purchaseConfirm === item.id;
                  return (
                    <div key={item.id} className={`bg-white border-2 rounded-3xl p-6 shadow-sm flex flex-col items-center gap-4 transition-all ${owned ? 'border-green-300 bg-green-50' : canAfford ? 'border-purple-100' : 'border-gray-100 opacity-70'}`}>
                      <BorderPreview item={item} small />
                      <p className="font-bold text-gray-800 text-lg text-center leading-tight">{item.name}</p>
                      <p className="text-gray-400 text-[10px] text-center">{item.description}</p>
                      <div className="flex items-center gap-1">
                        <SkillCoin size={14} />
                        <span className="text-gray-700 text-lg font-bold">{item.price}</span>
                      </div>
                      {owned ? (
                        <Badge className={`text-[10px] ${justBought ? 'bg-green-600 animate-pulse' : 'bg-green-500'}`}>
                          {justBought ? 'âœ“ Got it!' : 'Owned âœ“'}
                        </Badge>
                      ) : canAfford ? (
                        <Button onClick={() => handleBuyBorder(item)} className="bg-purple-500 hover:bg-purple-600 text-white rounded-full h-7 text-lg font-bold w-full active:scale-95">
                          Buy
                        </Button>
                      ) : (
            ...
            <div className="bg-yellow-50 border-2 border-yellow-200 rounded-3xl p-6">
              <div className="flex items-start gap-5">
                ...
                <div>
                  <p className="text-yellow-700 font-bold text-lg">ðŸ’¡ Earn more SC</p>
                  <p className="text-yellow-600 text-lg mt-1">Complete daily quests to earn SkillCoins. Visit your Profile to equip a border!</p>
                </div>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="robux" className="mt-5">
            ...
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-3xl p-5 mb-5 shadow-xl">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-gray-300 text-lg">Convert your SkillCoins into Robux!</p>
                  <p className="text-yellow-400 text-lg mt-1 font-semibold">Earn SC â†’ Exchange for R$</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border-2 border-gray-100 rounded-2xl p-5 mb-4 flex items-center gap-5 shadow-sm">
              <div className="flex items-center gap-1">
                <SkillCoin size={20} />
                <span className="text-gray-600 text-lg font-bold">5 SC</span>
              </div>
              <span className="text-gray-400 text-lg font-bold">â‰ˆ</span>
              <div className="flex items-center gap-1">
                <RobuxBadge size={18} />
                <span className="text-gray-600 text-lg font-bold">1 R$</span>
              </div>
              <span className="text-gray-400 text-lg ml-auto">More = better rate</span>
            </div>
            ...
            <div className="space-y-3">
              {ROBUX_ITEMS.map(item => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`bg-white border-2 rounded-3xl p-6 shadow-md transition-all ${
                  ...
                  >
                    {item.popular && (
                      <div className="flex justify-end mb-1">
                        <Badge className="bg-yellow-400 text-yellow-900 text-[10px] px-2">ðŸ”¥ Most Popular</Badge>
                      </div>
                    )}
                    <div className="flex items-center gap-6">
                      <div className="flex flex-col items-center gap-1 flex-shrink-0">
                        <RobuxIcon size={44} />
                        <p className="text-gray-800 font-black text-lg">{item.robux.toLocaleString()}</p>
                      ...
                      <div className="flex-1">
                        <p className="text-gray-700 font-bold text-lg mb-1">
                          ...
                          {item.bonus && <span className="text-green-500 text-lg ml-1">({item.bonus})</span>}
                        </p>
                        <div className="flex items-center gap-1.5 mb-3">
                          <SkillCoin size={16} />
                          <span className="text-gray-600 text-lg font-bold">{item.scCost.toLocaleString()} SC</span>
                        </div>
                        {owned ? (
                          <Badge className={`text-lg ${justBought ? 'bg-green-600 animate-pulse' : 'bg-green-500'}`}>
                            {justBought ? 'âœ“ Redeemed!' : 'Redeemed âœ“'}
                          </Badge>
                        ) : canAfford ? (
                          <Button
                            onClick={() => handleBuyRobux(item)}
                            className="w-full bg-gray-900 hover:bg-black text-white rounded-2xl h-9 font-bold text-lg flex items-center justify-center gap-4 active:scale-95"
                          >
                            <RobuxBadge size={16} />
                            Get {item.robux.toLocaleString()} Robux
                          </Button>
                        ) : (
                          <div className="flex items-center gap-1 text-gray-400 text-lg">
                            <Lock size={12} />
            ...
            <div className="bg-gray-900 rounded-3xl p-6 mt-5">
              <div className="flex items-start gap-5">
                <RobuxBadge size={24} />
                <div>
                  <p className="text-white font-bold text-lg">How it works</p>
                  <p className="text-gray-400 text-lg mt-1">Complete quests to earn SkillCoins, then exchange them here for Robux. The bigger the pack, the better the rate!</p>
                </div>
              </div>
            </div>
          </TabsContent>
