...
const FACE_OPTIONS = [
  { id: 'f1', label: 'Happy', base: 'ðŸ˜Š' },
  { id: 'f2', label: 'Cool', base: 'ðŸ˜Ž' },
  { id: 'f3', label: 'Excited', base: 'ðŸ¤©' },
  { id: 'f4', label: 'Silly', base: 'ðŸ˜œ' },
  { id: 'f5', label: 'Calm', base: 'ðŸ˜Œ' },
  { id: 'f6', label: 'Brave', base: 'ðŸ˜¤' },
];

const HAIR_OPTIONS = [
  { id: 'h1', label: 'Short', emoji: 'ðŸ‘¦', color: '#8B4513' },
  { id: 'h2', label: 'Long', emoji: 'ðŸ‘§', color: '#DAA520' },
  { id: 'h3', label: 'Curly', emoji: 'ðŸ§‘â€ðŸ¦±', color: '#4B0082' },
  { id: 'h4', label: 'Blonde', emoji: 'ðŸ‘±', color: '#FFD700' },
  { id: 'h5', label: 'Silver', emoji: 'ðŸ§‘â€ðŸ¦³', color: '#C0C0C0' },
  { id: 'h6', label: 'Bald', emoji: 'ðŸ§‘â€ðŸ¦²', color: '#FFA07A' },
];

const BODY_OPTIONS = [
  { id: 'b1', label: 'Casual', emoji: 'ðŸ‘•', bg: '#3b82f6' },
  { id: 'b2', label: 'Formal', emoji: 'ðŸ‘”', bg: '#1e3a8a' },
  { id: 'b3', label: 'Sport', emoji: 'ðŸŽ½', bg: '#ef4444' },
  { id: 'b4', label: 'Dress', emoji: 'ðŸ‘—', bg: '#ec4899' },
  { id: 'b5', label: 'Hoodie', emoji: 'ðŸ§¥', bg: '#6b7280' },
  { id: 'b6', label: 'Hero', emoji: 'ðŸ¦¸', bg: '#f59e0b' },
];

const ACCESSORY_OPTIONS = [
  { id: 'a1', label: 'None', emoji: 'âœ–ï¸' },
  { id: 'a2', label: 'Crown', emoji: 'ðŸ‘‘' },
  { id: 'a3', label: 'Hat', emoji: 'ðŸŽ©' },
  { id: 'a4', label: 'Glasses', emoji: 'ðŸ•¶ï¸' },
  { id: 'a5', label: 'Bow', emoji: 'ðŸŽ€' },
  { id: 'a6', label: 'Wizard', emoji: 'ðŸ§™' },
];
...
const SKIN_TONES = ['ðŸŸ¤', 'ðŸŸ ', 'ðŸŸ¡', 'âšª', 'ðŸ”µ'];
    ...
    switch (activeCategory) {
      case 0: // Face
        return (
          <div>
            {/* Skin tone row */}
            <p className="text-gray-500 text-lg font-semibold mb-2 uppercase tracking-wide">Skin Tone</p>
            ...
            <p className="text-gray-500 text-lg font-semibold mb-2 uppercase tracking-wide">Expression</p>
            <div className="grid grid-cols-3 gap-5">
              {FACE_OPTIONS.map(face => (
                <button key={face.id} onClick={() => handleSelect('face', face.id)}
                  className={`rounded-2xl p-6 flex flex-col items-center gap-1 border-2 transition-all active:scale-95 ${
                  ...
                  <span className="text-lg text-gray-500">{face.label}</span>
          ...
          <div className="grid grid-cols-3 gap-5">
            {HAIR_OPTIONS.map(hair => (
              <button key={hair.id} onClick={() => handleSelect('hair', hair.id)}
                ...
                <span className="text-lg text-gray-500">{hair.label}</span>
          ...
          <div className="grid grid-cols-3 gap-5">
            {BODY_OPTIONS.map(body => (
              <button key={body.id} onClick={() => handleSelect('body', body.id)}
                className={`rounded-2xl p-6 flex flex-col items-center gap-4 border-2 transition-all active:scale-95 ${
                ...
                <span className="text-lg text-gray-500">{body.label}</span>
          ...
          <div className="grid grid-cols-3 gap-5">
            {ACCESSORY_OPTIONS.map(acc => (
              <button key={acc.id} onClick={() => handleSelect('accessory', acc.id)}
                ...
                <span className="text-lg text-gray-500">{acc.label}</span>
            ...
            {BG_OPTIONS.map(bg => (
              <button key={bg.id} onClick={() => handleSelect('bg', bg.id)}
                ...
                <div className={`w-full h-full bg-gradient-to-br ${bg.color} flex items-center justify-center`}>
                  <span className="text-white text-lg font-bold">{bg.label}</span>
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
          {CATEGORIES.map((cat, i) => (
            <button key={i} onClick={() => setActiveCategory(i)}
              className={`flex-shrink-0 px-4 py-2 rounded-full text-lg font-semibold transition-all ${
                activeCategory === i
