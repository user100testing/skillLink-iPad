...
export function ChildProfileScreen({ onBack }: ChildProfileScreenProps) {
  ...
  const recentAchievements = [
    { name: 'Early Bird', emoji: 'ðŸŒ…', date: 'Oct 1, 2025' },
    { name: 'Bookworm', emoji: 'ðŸ“š', date: 'Sep 28, 2025' },
    { name: 'Math Wizard', emoji: 'ðŸ§™â€â™‚ï¸', date: 'Sep 25, 2025' },
  ];
  ...
  const favoriteCategories = [
    { name: 'Creativity', icon: 'ðŸŽ¨', count: 15 },
    { name: 'Life Skills', icon: 'ðŸŽ¯', count: 12 },
    { name: 'Healthy Habits', icon: 'ðŸ’ª', count: 10 },
  ];
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 pt-12 pb-8 rounded-b-[3rem] shadow-lg">
        ...
        <div className="grid grid-cols-3 gap-5">
          <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 text-center">
            ...
            <p className="text-indigo-100 text-lg">Points</p>
          </div>
          <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 text-center">
          ...
          <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 text-center">
            ...
            <p className="text-indigo-100 text-lg">Rank</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 mt-6">
        ...
        <div className="space-y-3">
          {recentAchievements.map((achievement, idx) => (
            <div key={idx} className="bg-gradient-to-r from-yellow-100 to-yellow-200 border-2 border-yellow-400 rounded-2xl p-6 flex items-center gap-6">
              <div className="text-4xl">{achievement.emoji}</div>
              <div className="flex-1">
                <p className="text-slate-700">{achievement.name}</p>
                <p className="text-slate-500 text-lg">{achievement.date}</p>
              </div>
      ...
      <div className="px-6 mt-6">
        ...
        <div className="space-y-3">
          {favoriteCategories.map((category, idx) => (
            <div key={idx} className="bg-white border-2 border-blue-100 rounded-2xl p-6 flex items-center gap-6">
              <div className="text-3xl">{category.icon}</div>
              <div className="flex-1">
                <p className="text-slate-700">{category.name}</p>
                <p className="text-slate-400 text-lg">{category.count} activities completed</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {recentQuests.map((quest, idx) => (
            <div key={idx} className="bg-green-50 border-2 border-green-200 rounded-2xl p-5 flex items-center gap-5">
              ...
              <div className="flex-1">
                <p className="text-slate-700 text-lg">{quest.name}</p>
                <p className="text-slate-400 text-lg">{quest.date}</p>
              </div>
              <Badge className="bg-green-500 text-lg">+{quest.points}</Badge>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 mb-6">
        <Card className="p-5 bg-gradient-to-r from-blue-100 to-indigo-100 border-2 border-blue-300">
          ...
          <ul className="space-y-2 text-slate-700">
            <li>â€¢ Most active time: Afternoons (3-5 PM)</li>
            <li>â€¢ Favorite activity type: Creative quests</li>
            <li>â€¢ Best day: Fridays (avg 140 points)</li>
            <li>â€¢ Learning streak: 7 consecutive days ðŸ”¥</li>
          </ul>
        </Card>
      </div>
    </div>
