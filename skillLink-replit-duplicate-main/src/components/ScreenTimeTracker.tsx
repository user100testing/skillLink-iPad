...
export function ScreenTimeTracker({ onNavigate }: ScreenTimeTrackerProps) {
  ...
  const getStatus = () => {
    if (hoursOff >= 6) return { color: 'from-green-400 to-emerald-500', status: 'Amazing!', emoji: 'ðŸŒŸ' };
    if (hoursOff >= 3) return { color: 'from-blue-400 to-cyan-500', status: 'Great!', emoji: 'ðŸ’ª' };
    if (hoursOff >= 1) return { color: 'from-yellow-400 to-orange-400', status: 'Good Start', emoji: 'ðŸ‘' };
    return { color: 'from-orange-400 to-red-400', status: 'Keep Going', emoji: 'ðŸŽ¯' };
  };
    ...
    <div className={`bg-gradient-to-r ${status.color} rounded-3xl p-5 shadow-lg`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/90 text-lg">Off Brainrot Content {status.emoji}</p>
          </div>
        </div>
      ...
      <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 mb-3">
        ...
        <div className="mt-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-white text-lg">Next reward at {nextReward.hours}h</span>
            <span className="text-white text-lg">+{nextReward.points} pts</span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        <div className="bg-white/20 backdrop-blur-sm rounded-xl p-5 text-center">
          ...
          <p className="text-white text-lg">Points Earned</p>
          <p className="text-white">+{pointsEarned}</p>
        </div>
        <div className="bg-white/20 backdrop-blur-sm rounded-xl p-5 text-center">
          <Zap className="text-yellow-300 mx-auto mb-1" size={20} />
          <p className="text-white text-lg">Daily Goal</p>
      ...
      <div className="mt-3 text-center">
        <p className="text-white/80 text-lg">
          {hoursOff >= 6 ? "You're a Focus Champion! ðŸ†" :
           hoursOff >= 3 ? "Keep it up! You're doing great! ðŸ’ª" :
           hoursOff >= 1 ? "Good start! Stay focused! ðŸŽ¯" :
           "Every minute counts! You got this! ðŸŒŸ"}
        </p>
      </div>
    </div>
  );
