...
const INITIAL_SUBTASKS: SubTask[] = [
  { id: 'floor', title: 'Clear the Floor', description: 'Pick up everything off the floor and put it away.', emoji: 'ðŸ§¹', points: 8, completed: false },
  { id: 'bed', title: 'Make the Bed', description: 'Straighten your sheets, blankets, and fluff your pillow.', emoji: 'ðŸ›ï¸', points: 6, completed: false },
  { id: 'desk', title: 'Organize Desk', description: 'Clear your desk, put away books and supplies neatly.', emoji: 'ðŸ“š', points: 7, completed: false },
  { id: 'clothes', title: 'Sort Clothes', description: 'Put dirty clothes in the hamper and fold clean ones.', emoji: 'ðŸ‘•', points: 6, completed: false },
  { id: 'shelves', title: 'Tidy Shelves', description: 'Arrange books, toys, and decorations on shelves.', emoji: 'ðŸ—‚ï¸', points: 5, completed: false },
  { id: 'dust', title: 'Wipe Surfaces', description: 'Give surfaces a quick wipe to remove dust.', emoji: 'âœ¨', points: 5, completed: false },
  { id: 'photo', title: 'Take Proof Photo', description: 'Snap a photo to show how clean your room is!', emoji: 'ðŸ“¸', points: 8, completed: false },
];
  ...
  return (
    <div className={`rounded-3xl p-5 mb-5 border-2 transition-all ${finished ? 'bg-green-50 border-green-300 dark:bg-green-950/20 dark:border-green-700' : running ? 'bg-blue-50 border-blue-300 dark:bg-blue-950/20 dark:border-blue-700' : 'bg-card border-border'}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        {!running && !finished && (
          <span className="text-lg text-muted-foreground bg-muted px-2 py-1 rounded-full">Optional</span>
        )}
      </div>
      ...
      <div className={`w-full rounded-2xl py-5 flex flex-col items-center justify-center mb-3 transition-all ${finished ? 'bg-green-100 dark:bg-green-900/30' : running ? 'bg-blue-500' : 'bg-muted'}`}>
        <span className={`text-5xl font-bold font-mono ${finished ? 'text-green-600' : running ? 'text-white' : 'text-foreground'}`}>
          {finished ? 'ðŸ† Done!' : `${mins}:${secs.toString().padStart(2, '0')}`}
        </span>
        <span className={`text-lg mt-1 ${running ? 'text-blue-100' : 'text-muted-foreground'}`}>
          {finished ? 'Speed clean complete!' : running ? 'Racing the clock!' : 'Tap to race the clock âš¡'}
        </span>
      </div>
      ...
      <div className="flex gap-4">
        {!finished && (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-4 py-3 rounded-xl font-semibold transition-all ${running ? 'bg-orange-100 text-orange-600 border-2 border-orange-200 dark:bg-orange-950/30 dark:text-orange-400' : 'bg-blue-500 text-white'}`}
          >
...
export function CleanRoomQuestScreen({ onBack, onComplete }: CleanRoomQuestScreenProps) {
  ...
  const cleanlinessLabel = progress === 0 ? 'Disaster Zone ðŸŒªï¸' :
    progress < 30 ? 'Getting Started ðŸ§¹' :
    progress < 60 ? 'Making Progress! ðŸ‘' :
    progress < 85 ? 'Almost There! âœ¨' :
    progress < 100 ? 'Nearly Spotless! ðŸ’«' : 'Spotless! ðŸ†';
    ...
    return (
      <div className="h-full bg-gradient-to-b from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 flex flex-col items-center justify-center px-6 pb-20 overflow-y-auto">
        <div className="text-center">
          ...
          <h2 className="text-green-600 font-bold text-3xl mb-2">Room Conquered! ðŸŽ‰</h2>
          ...
          <div className="bg-white dark:bg-card border-4 border-yellow-300 rounded-3xl p-6 mb-5 shadow-xl">
            ...
            <div className="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-2xl p-6 mb-3">
              ...
              <p className="text-yellow-100 text-lg text-center">SC Coins Earned!</p>
            </div>
            {speedBonus && (
              <div className="bg-blue-100 dark:bg-blue-900/30 rounded-xl p-5 mb-3 border border-blue-200">
                <p className="text-blue-700 dark:text-blue-300 font-bold text-center">âš¡ Speed Clean Bonus: +15 pts!</p>
              </div>
            )}
            <div className="grid grid-cols-3 gap-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-2 text-center">
                  ...
                  <p className="text-lg text-muted-foreground mt-1">Star {i + 1}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className="h-full bg-background overflow-y-auto pb-24">
      ...
      <div className="bg-gradient-to-br from-orange-400 to-amber-500 px-6 pt-12 pb-8 rounded-b-[3rem] shadow-lg">
        ...
        <div className="text-center">
          ...
          <span className="inline-block bg-orange-700 text-white text-lg font-bold px-3 py-1 rounded-full mb-4">
            â­â­â­ Hard â€” Hero Quest
          </span>
      ...
      <div className="px-6 mt-5">
        ...
        <div className="bg-card border-2 border-border rounded-3xl p-5 mb-5 shadow-md">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-lg font-bold text-primary">{cleanlinessLabel}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-yellow-100 text-lg">{TOTAL_POINTS - earnedPoints} more available</p>
              </div>
            </div>
            <div className="bg-white/20 rounded-2xl px-3 py-2">
              <p className="text-white font-bold">{completedCount}/{subtasks.length}</p>
              <p className="text-yellow-100 text-lg">tasks</p>
            </div>
          </div>
        </div>
              ...
              {task.id === 'photo' ? (
                <div>
                  <div className="flex items-center gap-5 mb-3">
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`text-lg font-bold ${task.completed ? 'text-green-600' : 'text-primary'}`}>+{task.points} pts</span>
                  ...
                  {!task.completed && (
                    <label className="block cursor-pointer">
                      <div className="border-2 border-dashed border-blue-300 dark:border-blue-700 rounded-2xl p-6 text-center bg-blue-50 dark:bg-blue-950/20 active:bg-blue-100 transition-colors">
                        {proofPhoto ? (
                          <><CheckCircle className="text-green-500 mx-auto mb-1" size={28} /><p className="text-green-700 font-semibold text-lg">{proofPhoto.name}</p></>
                        ) : (
                          <><Camera className="text-blue-400 mx-auto mb-1" size={28} /><p className="text-blue-600 font-semibold text-lg">Tap to upload proof photo</p></>
                        )}
                      </div>
        ...
        {allDone ? (
          ...
          >
            ðŸ† Complete Quest! (+{earnedPoints + (speedBonus ? 15 : 0)} pts)
          </button>
        ) : (
          <div className="bg-card border-2 border-border rounded-3xl p-6 text-center">
            <p className="text-muted-foreground text-lg">Complete {subtasks.length - completedCount} more task{subtasks.length - completedCount !== 1 ? 's' : ''} to finish!</p>
