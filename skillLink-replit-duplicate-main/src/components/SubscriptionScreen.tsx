...
export function SubscriptionScreen({ onBack, currentPlan = 'none', onSubscribe }: SubscriptionScreenProps) {
  ...
  const plans = {
    base: {
      ...
      features: [
        { icon: 'ðŸ‘¨â€ðŸ‘©â€ðŸ‘§', text: 'Unlimited Parent/Guardian accounts' },
        { icon: 'ðŸ‘¶', text: '1 Child account' },
        { icon: 'ðŸŽ¯', text: 'Unlimited daily quests' },
        { icon: 'ðŸ“š', text: 'Access to all courses' },
        { icon: 'ðŸ†', text: 'Full leaderboard access' },
        { icon: 'ðŸ“Š', text: 'Parent progress dashboard' },
        { icon: 'ðŸŽ¨', text: 'All avatar customizations' },
      ],
      notIncluded: [
    ...
    family: {
      ...
      features: [
        { icon: 'ðŸ‘¨â€ðŸ‘©â€ðŸ‘§', text: 'Unlimited Parent/Guardian accounts' },
        { icon: 'ðŸ‘¶ðŸ‘¶', text: '2 Child accounts' },
        { icon: 'ðŸŽ¯', text: 'Unlimited daily quests' },
        { icon: 'ðŸ“š', text: 'Access to all courses' },
        { icon: 'ðŸ†', text: 'Full leaderboard access' },
        { icon: 'ðŸ“Š', text: 'Parent progress dashboard' },
        { icon: 'ðŸŽ¨', text: 'All avatar customizations' },
        { icon: 'ðŸ‘¨â€ðŸ‘©â€ðŸ‘§â€ðŸ‘¦', text: 'Family challenge mode' },
        { icon: 'ðŸ’°', text: '2x SkillCoin bonus on quests' },
      ],
      notIncluded: [],
      badge: 'BEST VALUE',
    },
    ...
    return (
      <div className="h-full bg-gradient-to-b from-purple-500 to-pink-500 flex flex-col items-center justify-center px-8">
        <div className="text-center text-white">
          ...
          <p className="text-purple-200 text-lg mb-8">All premium features are now unlocked</p>
    ...
    <div className="h-full bg-gray-50 overflow-y-auto pb-8">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-pink-500 px-6 pt-12 pb-8 rounded-b-[3rem] shadow-xl">
        ...
        <div className="text-center">
          <div className="inline-flex items-center gap-4 bg-white/20 rounded-full px-4 py-1 mb-3">
            ...
            <span className="text-white text-lg font-semibold">Premium Plans</span>
          </div>
          <h1 className="text-white text-3xl font-black mb-2">Unlock Everything</h1>
          <p className="text-purple-100 text-lg">Choose the plan that fits your family</p>
        </div>
      </div>
      ...
      <div className="px-6 mt-6">
        <div className="bg-white rounded-2xl p-1 flex shadow-md border border-gray-100">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-xl text-lg font-bold transition-all flex items-center justify-center gap-4 ${
              billing === 'yearly' ? 'bg-purple-500 text-white shadow-md' : 'text-gray-500'}`}>
            Yearly
            <span className={`text-lg px-2 py-0.5 rounded-full font-black ${
            ...
            <button
              ...
              <div className={`bg-gradient-to-r ${plan.color} px-5 py-4 flex items-center justify-between`}>
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    {(plan as any).badge && (
                      <span className="bg-yellow-400 text-yellow-900 text-lg font-black px-2 py-0.5 rounded-full">
                        {(plan as any).badge}
                      </span>
                    )}
                  </div>
                ...
                <div className="text-right">
                  <p className="text-white text-3xl font-black">â‚¬{getPrice(planKey)}</p>
                  <p className="text-white/70 text-lg">/month</p>
                </div>
              </div>
              ...
              <div className={`px-5 py-4 ${isSelected ? plan.lightColor : 'bg-white'}`}>
                <div className="space-y-2">
                  {plan.features.map((feature, i) => (
                    <div key={i} className="flex items-center gap-5">
                      ...
                      <span className="text-gray-700 text-lg">{feature.icon} {feature.text}</span>
                    </div>
                  ))}
                  {plan.notIncluded.map((feature, i) => (
                    <div key={i} className="flex items-center gap-5 opacity-40">
                      ...
                      <span className="text-gray-400 text-lg line-through">{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className={`mt-3 flex items-center justify-between ${isSelected ? 'opacity-100' : 'opacity-0'}`}>
                  <span className={`text-lg font-bold ${plan.textColor}`}>Selected âœ“</span>
                  {isCurrent && <span className="text-lg text-gray-400 bg-gray-100 rounded-full px-3 py-1">Current plan</span>}
                </div>
              </div>
            </button>
          );
      ...
      <div className="px-6 mt-4">
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <p className="text-gray-500 text-lg font-bold uppercase mb-3">Plan Comparison</p>
          <div className="flex">
            <div className="flex-1">
              <p className="text-lg text-gray-400 mb-2">Feature</p>
              {['Parent accounts', 'Child accounts', 'Family challenges', 'Bonus SC coins'].map(f => (
                <p key={f} className="text-lg text-gray-600 py-1 border-b border-gray-50">{f}</p>
              ))}
            </div>
            <div className="w-16 text-center">
              <p className="text-lg text-blue-500 font-bold mb-2">Base</p>
              {['Unlimited', '1', 'âœ•', 'â€”'].map((v, i) => (
                <p key={i} className={`text-lg py-1 border-b border-gray-50 ${v === 'âœ•' ? 'text-gray-300' : 'text-blue-600 font-semibold'}`}>{v}</p>
              ))}
            </div>
            <div className="w-16 text-center">
              <p className="text-lg text-purple-500 font-bold mb-2">Family</p>
              {['Unlimited', '2', 'âœ“', '2x'].map((v, i) => (
                <p key={i} className={`text-lg py-1 border-b border-gray-50 ${v === 'âœ“' || v === '2x' ? 'text-green-500 font-bold' : 'text-purple-600 font-semibold'}`}>{v}</p>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="px-6 mt-6">
        <Button
          ...
          {subscribing ? (
            <span className="flex items-center gap-4"><span className="animate-spin">â³</span> Processing...</span>
          ) : (
            <span className="flex items-center gap-4">
              <Crown size={20} />
              Subscribe to {plans[selected].name} â€” â‚¬{getPrice(selected)}/mo
              <ChevronRight size={20} />
            </span>
          )}
        </Button>
        <p className="text-center text-gray-400 text-lg mt-3">Cancel anytime Â· Secure payment Â· No hidden fees</p>
      </div>
