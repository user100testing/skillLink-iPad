...
export function ParentDashboardScreen({ onViewChildProfile, onViewVideoReviews, onNavigate, userProfile, skillLevels, weeklyEvent, language = 'en' }: ParentDashboardScreenProps) {
  ...
  // Mock data for rewards â€” names/costs match the Shop
  ...
  const weeklyTips = [
    "Encourage your child to try a new skill this week! ðŸŒŸ",
    "Family challenges build stronger bonds - try one together! ðŸ‘¨â€ðŸ‘©â€ðŸ‘§",
    "Celebrate small wins to keep motivation high! ðŸŽ‰",
    "Consistency is key - help maintain that daily streak! ðŸ”¥",
    "Mix creative and learning quests for balanced growth! ðŸ“šðŸŽ¨"
  ];
    ...
    <div className="h-full bg-gradient-to-b from-indigo-50 to-white dark:from-slate-900 dark:to-slate-800 pb-20 overflow-y-auto">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 pt-12 pb-8 rounded-b-[3rem] shadow-lg">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-5">
            <span className="text-4xl">{userProfile?.avatar || 'ðŸ‘¨â€ðŸ’¼'}</span>
            <div>
              <h2 className="text-white">Welcome, {userProfile?.name || 'Parent'}!</h2>
              <p className="text-indigo-100 text-lg">Manage your child's learning journey</p>
            </div>
          </div>
          {onNavigate && (
            <button 
        ...
        <div className="flex gap-5 mt-6 mb-4">
          ...
          >
            <User size={18} />
            <span className="text-lg">Child Profile</span>
          </button>
          <button 
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 text-center">
            ...
            <p className="text-indigo-100 text-lg">Streak</p>
          </div>
          <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 text-center">
      ...
      <div className="px-6 mt-6">
        <div className="bg-gradient-to-r from-yellow-400 to-orange-400 rounded-3xl p-6 shadow-lg">
          ...
          <p className="text-white text-lg">â­ {weekHighlight}</p>
        </div>
      </div>
          ...
          <div className="bg-gradient-to-r from-green-100 to-emerald-100 dark:from-green-900/30 dark:to-emerald-900/30 border-2 border-green-300 dark:border-green-700 rounded-3xl p-5">
            ...
            <div className="grid grid-cols-2 gap-5 mb-4">
              <div className="bg-white dark:bg-slate-800 rounded-xl p-5">
                <p className="text-slate-600 dark:text-slate-400 text-lg mb-1">Quests Completed</p>
                <p className="text-green-700 dark:text-green-300 text-xl">{weeklySummary.questsCompleted}</p>
                <p className="text-green-600 dark:text-green-400 text-lg mt-1">â†‘ {weeklySummary.lastWeekQuests} last week</p>
              </div>
              <div className="bg-white dark:bg-slate-800 rounded-xl p-5">
                <p className="text-slate-600 dark:text-slate-400 text-lg mb-1">Team Challenges</p>
                <p className="text-green-700 dark:text-green-300 text-xl">{weeklySummary.teamChallenges}</p>
                <p className="text-green-600 dark:text-green-400 text-lg mt-1">Family bonding! ðŸ‘¨â€ðŸ‘©â€ðŸ‘§</p>
              </div>
              <div className="bg-white dark:bg-slate-800 rounded-xl p-5">
                <p className="text-slate-600 dark:text-slate-400 text-lg mb-1">Points Earned</p>
                <p className="text-green-700 dark:text-green-300 text-xl">{weeklySummary.pointsEarned}</p>
                <p className="text-green-600 dark:text-green-400 text-lg mt-1">â†‘ {weeklySummary.lastWeekPoints} last week</p>
              </div>
              <div className="bg-white dark:bg-slate-800 rounded-xl p-5">
                <p className="text-slate-600 dark:text-slate-400 text-lg mb-1">Current Streak</p>
                <p className="text-green-700 dark:text-green-300 text-xl">{weeklySummary.currentStreak} days</p>
                <p className="text-green-600 dark:text-green-400 text-lg mt-1">ðŸ”¥ On fire!</p>
              </div>
            </div>
      ...
      <div className="px-6 mt-6">
        <Card className="p-5 border-2 border-blue-100 dark:border-blue-900">
          ...
          <div className="grid grid-cols-2 gap-6 mb-4">
            <div className={`rounded-2xl p-6 ${questGrowth >= 0 ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
              <div className="flex items-center gap-4 mb-2">
                {questGrowth >= 0 ? (
              ...
              <p className="text-slate-600 dark:text-slate-400 text-lg">More quests completed</p>
              {questGrowth >= 10 && <p className="text-green-600 dark:text-green-400 text-lg mt-1">Amazing progress! ðŸ‘</p>}
            </div>

            <div className={`rounded-2xl p-6 ${pointsGrowth >= 0 ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
              <div className="flex items-center gap-4 mb-2">
                {pointsGrowth >= 0 ? (
                  <TrendingUp className="text-green-600 dark:text-green-400" size={20} />
                ) : (
              ...
              <p className="text-slate-600 dark:text-slate-400 text-lg">More points earned</p>
              {pointsGrowth >= 10 && <p className="text-green-600 dark:text-green-400 text-lg mt-1">Keep it up! ðŸŒŸ</p>}
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="px-6 mt-6">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          >
            ...
            <p className="text-lg">Create Custom Quest</p>
          </button>
          ...
          >
            ...
            <p className="text-lg">Reward Requests</p>
            {rewardRequests.filter(r => r.status === 'pending').length > 0 && (
              <div className="absolute -top-2 -right-2 bg-orange-500 rounded-full w-6 h-6 flex items-center justify-center">
                <span className="text-white text-lg">{rewardRequests.filter(r => r.status === 'pending').length}</span>
              </div>
            )}
          </button>
        </div>
      ...
      <div className="px-6 mt-6">
        ...
        >
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <div className="flex items-center gap-4 mb-0.5">
                  <p className="text-white font-black text-lg">Upgrade Your Plan</p>
                  <span className="bg-yellow-400 text-yellow-900 text-[10px] font-black px-2 py-0.5 rounded-full">BEST VALUE</span>
                </div>
                <p className="text-purple-100 text-lg">Family Plan â€” 2 kids Â· family challenges Â· 2Ã— SC</p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-6 mt-4 pt-4 border-t border-white/20">
            <div className="flex items-center gap-1.5 text-purple-100 text-lg">
        ...
        <Card className="p-5 border-2 border-indigo-100 dark:border-indigo-900">
            ...
            {weeklyData.map((day, idx) => (
              <div key={idx} className="flex-1 flex flex-col items-center gap-4">
                <div className="flex-1 flex items-end w-full gap-1">
                  ...
                  >
                    <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-indigo-600 text-white px-2 py-1 rounded text-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                      {day.points} pts
                    </div>
                  </div>
                </div>
                <p className="text-slate-600 dark:text-slate-400 text-lg">{day.day}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-6 mb-3">
            <div className="flex items-center gap-4">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-lg">Last Week</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-4 h-4 bg-indigo-500 dark:bg-indigo-400 rounded" />
              <p className="text-slate-600 dark:text-slate-400 text-lg">This Week</p>
            </div>
          </div>

          <div className="bg-indigo-50 dark:bg-indigo-900/30 rounded-2xl p-5 text-center">
            <p className="text-indigo-700 dark:text-indigo-300">Average: 110 points/day ðŸ“ˆ</p>
          </div>
        </Card>
      </div>
      ...
      <div className="px-6 mt-6">
        <Card className="p-5 border-2 border-blue-100 dark:border-blue-900">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-lg">2 quests completed, 5 remaining</p>
        </Card>
      </div>
          ...
          {recentQuests.map((quest, idx) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-400 dark:text-slate-500 text-lg">{quest.date}</p>
              </div>
                  ...
                  <div className="flex items-start gap-5">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        <SkillCoin size={14} />
                        <span className="text-slate-500 dark:text-slate-400 text-lg font-bold">{reward.pointsCost.toLocaleString()} SC</span>
                        <span className="text-slate-400 dark:text-slate-500 text-lg">â€¢ {reward.requestedAt}</span>
                      </div>
                      <div className="mt-2">
                        {reward.status === 'pending' && (
                        ...
                        {reward.status === 'approved' && (
                          <Badge className="bg-green-500">Approved âœ“</Badge>
                        )}
