...
// Dynamic Timer Component â€” click to start/stop, hold to reset
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-3xl p-5 mb-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-blue-700 font-bold">â±ï¸ Timer</h3>
        <button
          onClick={toggleMode}
          className="text-lg text-blue-500 border border-blue-300 rounded-full px-3 py-1"
        >
          {mode === 'countdown' ? 'â†“ Countdown' : 'â†‘ Count Up'}
        </button>
      </div>
      ...
      >
        <span className={`text-5xl font-bold font-mono ${
          ...
          {isFinished ? 'âœ… Done!' : display}
        </span>
        <span className={`text-lg mt-1 ${running ? 'text-blue-100' : 'text-blue-400'}`}>
      ...
      <div className="flex gap-4">
        <button
          onClick={toggle}
          className={`flex-1 flex items-center justify-center gap-4 py-3 rounded-xl font-semibold transition-all ${
...
export function QuestDetailScreen({ questId, onBack, onStartChallenge, onSoloSubmit, questData: propQuestData }: QuestDetailScreenProps) {
  ...
  const defaultQuestData: Record<number, any> = {
    1: { title: 'Brush your teeth', icon: 'ðŸª¥', points: 10, instructions: ['Get your toothbrush and toothpaste', 'Put a pea-sized amount on the brush', 'Brush for 2 minutes (all teeth!)', 'Rinse your mouth with water', 'Put everything away neatly'], tips: 'Brush twice a day - morning and night!', color: 'from-[#2563eb] to-[#1d4ed8]', timerSeconds: 120 },
    3: { title: 'Draw your favorite animal', icon: 'ðŸŽ¨', points: 25, instructions: ['Get paper and drawing tools', 'Think about your favorite animal', 'Start with simple shapes', 'Add details like eyes, ears, tail', 'Color it in if you want!', 'Show it to someone you love'], tips: "Don't worry about making it perfect!", color: 'from-purple-400 to-purple-500', timerSeconds: 600 },
    6: { title: 'Do 10 jumping jacks', icon: 'ðŸ¤¸', points: 10, instructions: ['Stand up straight with feet together', 'Jump and spread your feet, raise arms', 'Jump again, bring feet together arms down', 'Do this 10 times!', 'Take a deep breath when done'], tips: 'Count out loud to make it fun!', color: 'from-green-400 to-green-500', timerSeconds: 60 },
  };
      ...
      <div className="h-full bg-gradient-to-b from-blue-50 to-white pb-20 overflow-y-auto">
        ...
        <div className="px-6 mt-6">
          ...
          <div className="bg-white border-2 border-blue-200 rounded-3xl p-6 mb-4 shadow-md">
            <h3 className="text-blue-600 font-bold mb-4">ðŸ“¸ Upload Your Proof</h3>
              ...
              <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${selectedFile ? 'border-green-400 bg-green-50' : 'border-blue-300 bg-blue-50'}`}>
                {selectedFile ? (
                  <>
                    ...
                    <p className="text-green-500 text-lg mt-1">Tap to change</p>
                  </>
                ) : (
                  <>
                    <Camera className="text-blue-400 mx-auto mb-2" size={40} />
                    <p className="text-blue-600 font-semibold">Tap to upload photo/video</p>
                    <p className="text-blue-400 text-lg mt-1">Show you completed the quest</p>
                  </>
                )}
              </div>
          ...
          <p className="text-center text-gray-400 text-lg mt-3">Your parent will review and approve</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-full bg-gradient-to-b from-green-50 to-white flex flex-col items-center justify-center px-6 pb-20">
        <div className="text-center">
          ...
          <h2 className="text-green-600 font-bold text-3xl mb-4">Quest Submitted! ðŸŽ‰</h2>
          <div className="bg-white border-4 border-green-300 rounded-3xl p-8 mb-6 shadow-xl">
            ...
            <div className="bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-2xl p-6">
              ...
              <p className="text-yellow-100 text-lg">SC Coins (pending approval)</p>
            </div>
          </div>
          <div className="bg-yellow-50 border-2 border-yellow-200 rounded-3xl p-5 mb-6">
            <p className="text-yellow-700 font-semibold">â³ Waiting for parent approval</p>
            <p className="text-yellow-600 text-lg mt-1">You'll get your coins once approved!</p>
          </div>
    ...
    <div className="h-full bg-gradient-to-b from-blue-50 to-white pb-20 overflow-y-auto">
      <div className={`bg-gradient-to-r ${quest.color} px-6 pt-12 pb-8 rounded-b-[3rem] shadow-lg`}>
        ...
        <div className="text-center">
          ...
          {quest.difficulty && (
            <div className="mb-3">
              <span className={`inline-block px-4 py-1 rounded-full text-white text-lg font-semibold ${quest.difficulty === 'easy' ? 'bg-green-500' : quest.difficulty === 'medium' ? 'bg-yellow-500' : 'bg-orange-500'}`}>
                {quest.difficulty === 'easy' && 'â­ Easy'}
                {quest.difficulty === 'medium' && 'â­â­ Medium'}
                {quest.difficulty === 'hard' && 'â­â­â­ Hard'}
              </span>
            </div>
          )}
          ...
          <div className="space-y-3">
            {quest.instructions.map((instruction: string, index: number) => (
              <div key={index} className="flex gap-5">
                <div className="bg-blue-500 text-white rounded-full w-7 h-7 flex items-center justify-center flex-shrink-0 mt-0.5 text-lg font-bold">
                  {index + 1}
                </div>
        ...
        >
          <div className="flex items-center gap-6">
            ...
            <div className="text-left">
              ...
              <p className="text-blue-100 text-lg">Complete on your own and upload proof</p>
            </div>
          </div>
        </button>
        ...
        >
          <div className="flex items-center gap-6">
            ...
            <div className="text-left">
              ...
              <p className="text-orange-100 text-lg">Do it with friends or parents ðŸŽ®</p>
            </div>
          </div>
        </button>
      </div>
