...
export function ChallengeModeScreen({ questId, questData, onBack, onSubmitVideo }: ChallengeModeScreenProps) {
  ...
  const availablePeople = [
    { id: 'mom', name: 'Mom', emoji: 'ðŸ‘©', type: 'parent' },
    { id: 'dad', name: 'Dad', emoji: 'ðŸ‘¨', type: 'parent' },
    { id: 'sibling1', name: 'Emma', emoji: 'ðŸ‘§', type: 'friend' },
    { id: 'friend1', name: 'Jake', emoji: 'ðŸ‘¦', type: 'friend' },
    { id: 'friend2', name: 'Mia', emoji: 'ðŸ‘§', type: 'friend' },
    { id: 'friend3', name: 'Liam', emoji: 'ðŸ‘¦', type: 'friend' },
  ];
    ...
    return (
      <div className="h-full bg-gradient-to-b from-purple-50 to-white flex flex-col items-center justify-center px-6 pb-20">
        <div className="text-center">
          ...
          <h2 className="text-purple-600 mb-4">Challenge Submitted! ðŸŽ‰</h2>
    ...
    <div className="h-full bg-gradient-to-b from-purple-50 to-white pb-20 overflow-y-auto">
      ...
      <div className="px-6 mt-6">
        <div className="bg-white border-2 border-purple-200 rounded-3xl p-6 shadow-md">
          <h3 className="text-purple-600 mb-4">ðŸ‘¥ Who do you want to do this with?</h3>
          <p className="text-slate-500 text-lg mb-4">Select friends or family members to challenge!</p>
          ...
          <div className="grid grid-cols-2 gap-5">
            {availablePeople.map((person) => (
              ...
              >
                ...
                <p className="text-slate-400 text-lg capitalize">{person.type}</p>
              </button>
            ))}
          </div>
          ...
          {selectedPeople.length > 0 && (
            <div className="mt-4 bg-purple-50 rounded-2xl p-5 text-center">
              <p className="text-purple-700">
                âœ¨ {selectedPeople.length} {selectedPeople.length === 1 ? 'person' : 'people'} selected
              </p>
            </div>
          )}
        </div>
      ...
      <div className="px-6 mt-6">
        <div className="bg-white border-2 border-pink-200 rounded-3xl p-6 shadow-md">
          <h3 className="text-pink-600 mb-4">ðŸ“¹ Prove you did it!</h3>
          <p className="text-slate-500 text-lg mb-4">Upload a video showing you completed the quest</p>
          ...
          {!videoFile ? (
            <label className="block">
              ...
              <div className="border-2 border-dashed border-pink-300 rounded-2xl p-8 text-center cursor-pointer hover:border-pink-500 hover:bg-pink-50 transition-all">
                ...
                <p className="text-slate-400 text-lg">Take a video with your device camera</p>
              </div>
            </label>
          ) : (
            <div className="space-y-3">
              <div className="bg-gradient-to-r from-green-100 to-green-50 border-2 border-green-300 rounded-2xl p-6 flex items-center gap-5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-green-600 text-lg">{videoFile.name}</p>
                </div>
              </div>
      ...
      <div className="px-6 mt-6">
        <div className="bg-blue-50 border-2 border-blue-200 rounded-3xl p-5">
          <h3 className="text-blue-700 mb-3">ðŸ“ Quest Steps:</h3>
          <div className="space-y-2">
            {questData.instructions.slice(0, 3).map((instruction: string, index: number) => (
              <div key={index} className="flex gap-4 items-start">
                <div className="bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center flex-shrink-0 text-lg">
                  {index + 1}
                </div>
                <p className="text-blue-600 text-lg flex-1">{instruction}</p>
              </div>
            ))}
            {questData.instructions.length > 3 && (
              <p className="text-blue-400 text-lg ml-8">...and more!</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="px-6 mt-8 mb-6">
        ...
        {!videoFile && (
          <p className="text-center text-slate-400 text-lg mt-3">
